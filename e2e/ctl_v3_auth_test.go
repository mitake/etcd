...
func authTestRevokeWithDelete(cx ctlCtx) {
	...
	// just delete the role without revoking
	...
	// create aother new role
	if err := ctlV3Role(cx, []string{"add", "test-role3"}, "Role test-role3 created"); err != nil {
		cx.t.Fatal(err)
	}

	// grant the new role to the user
	if err := ctlV3User(cx, []string{"grant-role", "test-user", "test-role3"}, "Role test-role3 is granted to user test-user", nil); err != nil {
		cx.t.Fatal(err)
	}

	// check the result
	if err := ctlV3User(cx, []string{"get", "test-user"}, "Roles: test-role test-role2 test-role3", nil); err != nil {
		cx.t.Fatal(err)
	}

	// delete the role with revoking
	if err := ctlV3Role(cx, []string{"delete", "--revoke", "test-role3"}, "Role test-role3 deleted"); err != nil {
		cx.t.Fatal(err)
	}
